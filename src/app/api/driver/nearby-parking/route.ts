...
export async function GET(request: NextRequest) {
  try {
    ...
    const processedAreas = await Promise.all(
      allParkingAreas.map(async (area) => {
        ...
        // Get all floors for this parking area
        const areaFloors = await db.select()
          .from(floors)
          .where(eq(floors.parkingAreaId, area.id));
        ...
        for (const floor of areaFloors) {
          const floorSlots = await db.select()
            .from(slots)
            .where(eq(slots.floorId, floor.id));

          totalSlots += floorSlots.length;
          availableSlots += floorSlots.filter(slot => slot.status === 'available').length;
        }
